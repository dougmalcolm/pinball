...
The final model lacks predictive value. With a model R<sup>2</sup> value of 0.276, and a predicted rating v. actual rating R<sup>2</sup> value of 0.290, there is simply too much unexplained variance that the model does not capture. Age Started, Total Events, and Years Active do affect rating to a statistically significant. The effects are just simply so small and variable it would be silly to try to accurately predict rating from these three metrics alone.
